...
이때는 `AspectJ`가 제공하는 특별한 컴파일러를 사용해야 한다.
...
궁금한 분은 `Java Instrumentation`를 검색해보자.
...
스프링과 같은 컨테이너의 도움을 받고 **프록시와 DI, 빈 포스트 프로세서 같은 개념들을 총 동원**해야 한다.
...
* 컴파일 시점
    * 실제 대상 코드에 Aspect를 통한 부가 기능 호출 코드가 포함된다.
    * AspectJ를 직접 사용해야 한다.
* 클래스 로딩 시점
    * 실제 대상 코드에 Aspect를 통한 부가 기능 호출 코드가 포함된다.
    * AspectJ를 직접 사용해야 한다.
...
>
> AspectJ를 사용하려면 공부할 내용도 많고,
> 자바 관련 설정(특별한 컴파일러, AspectJ 전용 문법, 자바 실행 옵션)도 복잡하다.
